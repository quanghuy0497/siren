...
class ROIHeads(torch.nn.Module):
    ...
    def _sample_proposals_ood(
        ...
        if has_gt:
            # breakpoint()
            gt_classes = torch.zeros(gt_classes[matched_idxs].size()).to('cuda:0')
            # Label unmatched proposals (0 label from matcher) as background (label=num_classes)
            # coco_classes == 80.
            gt_classes[matched_labels == 0] = self.num_classes
            # Label ignore proposals (-1 label)
...
class ROIHeadsLogisticGMMNew(ROIHeads):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.prototypes = torch.zeros((self.num_classes, self.projection_dim)).to('cuda:0')
        self.learnable_kappa = nn.Linear(self.num_classes, 1, bias=False).to('cuda:0')
    ...
    def _forward_box(self, features: Dict[str, torch.Tensor], proposals: List[Instances], iteration: int):
        ...
        if self.training:
            ...
            gt_classes_filtered = gt_classes[gt_classes != self.num_classes].to('cuda:0')

            projections_filtered = projections[gt_classes != self.num_classes].to('cuda:0')
            # projections_filtered = box_features[gt_classes != self.num_classes].to('cuda:0')
